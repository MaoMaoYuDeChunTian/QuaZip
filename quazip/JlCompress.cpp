...
bool JlCompress::compressDirExtend(QString fileCompressed, QString dir, QTextCodec* Codec)
{
	QuaZip zip(fileCompressed);
	...
	// Aggiungo i file e le sotto cartelle
	if (!compressSubDir(&zip, dir, dir, true)) {
		QFile::remove(fileCompressed);
		return false;
	}
...
QStringList JlCompress::extractDirExtend(QString fileCompressed, QString dir, QTextCodec* Codec)
{
	QuaZip zip(fileCompressed);
	zip.setFileNameCodec(Codec);
